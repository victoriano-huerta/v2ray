...
BASE_SOURCE_PATH="https://multi.victor.app"
...
removeV2Ray() {
    ...
    #卸载Xray脚本
    bash <(curl -L -s https://multi.victor.app/go.sh) --remove -x >/dev/null 2>&1
...
installDependent(){
    ...
    #install python3 & pip
    source <(curl -sL https://python3.victor.app/install.sh)
}
...
updateProject() {
    ...
    #安装V2ray主程序
    [[ ${INSTALL_WAY} == 0 ]] && bash <(curl -L -s https://multi.victor.app/go.sh)
}
